...
> **Crowe Network Note:** The corporate SSL proxy intercepts TLS. Prefix all Vercel CLI and curl commands with `NODE_TLS_REJECT_UNAUTHORIZED=0`. This is also required for `shadcn` registry calls.

#### GitHub Repo Creation (Crowe Network — no winget, no PAT needed)

The `gh` CLI is installed at `C:\Users\RachurA\AppData\Local\gh-cli\bin\gh.exe` (not in PATH). GitHub credentials are stored in Windows Credential Manager and can be retrieved with:

```bash
# Retrieve stored GitHub token from Windows Credential Manager
printf "protocol=https\nhost=github.com\n" | git credential fill
# Returns: username=achyuthrachur, password=<token>

# Use gh CLI (full path) to create repo and push in one command
GH="/c/Users/RachurA/AppData/Local/gh-cli/bin/gh.exe"
"$GH" repo create achyuthrachur/[name] --public --description "[desc]" --source=. --remote=origin --push
```

#### Vercel Setup + Blob Store

```bash
# Link project (Vercel auto-detects Next.js, connects GitHub repo)
NODE_TLS_REJECT_UNAUTHORIZED=0 vercel link --yes --project [name]

# Create and connect Vercel Blob store
NODE_TLS_REJECT_UNAUTHORIZED=0 vercel blob create-store [name]-blob
# → Note the store ID (store_XXXX) from output

# Connect blob store to project via API (interactive CLI broken in this env)
VTOKEN=$(cat "$APPDATA/com.vercel.cli/Data/auth.json" | node -e "process.stdin.resume();let d='';process.stdin.on('data',c=>d+=c);process.stdin.on('end',()=>{const j=JSON.parse(d);console.log(j.token||Object.values(j)[0])})")
TEAM_ID="team_jTMSsUBJBbOqgNTyjjsr9PY2"   # achyuth-rachurs-projects
PROJ_ID="<from .vercel/project.json>"
STORE_ID="<from create-store output>"

node -e "
process.env.NODE_TLS_REJECT_UNAUTHORIZED='0';
const https=require('https');
const token=process.env.VTOKEN||'$VTOKEN';
const data=JSON.stringify({projectId:'$PROJ_ID',environments:['production','preview','development']});
const req=https.request({hostname:'api.vercel.com',path:'/v1/storage/stores/$STORE_ID/connections?teamId=$TEAM_ID',method:'POST',headers:{'Authorization':'Bearer '+token,'Content-Type':'application/json','Content-Length':Buffer.byteLength(data)}},res=>{let b='';res.on('data',d=>b+=d);res.on('end',()=>console.log(res.statusCode,b.substring(0,200)))});req.write(data);req.end();
"

# Pull env vars (gets BLOB_READ_WRITE_TOKEN into .env.local)
NODE_TLS_REJECT_UNAUTHORIZED=0 vercel env pull .env.local --yes

# If BLOB_READ_WRITE_TOKEN missing from pull (only prod/preview, not dev):
# Get token value directly and add to .env.local manually:
ENV_ID=$(node -e "
process.env.NODE_TLS_REJECT_UNAUTHORIZED='0';
const https=require('https');
const req=https.request({hostname:'api.vercel.com',path:'/v9/projects/$PROJ_ID/env?teamId=$TEAM_ID',method:'GET',headers:{'Authorization':'Bearer $VTOKEN'}},res=>{let b='';res.on('data',d=>b+=d);res.on('end',()=>{const j=JSON.parse(b);(j.envs||[]).filter(e=>e.key.includes('BLOB')).forEach(e=>console.log(e.id))})});req.end();
")
BLOB_TOKEN=$(node -e "
process.env.NODE_TLS_REJECT_UNAUTHORIZED='0';
const https=require('https');
const req=https.request({hostname:'api.vercel.com',path:'/v9/projects/$PROJ_ID/env/$ENV_ID?teamId=$TEAM_ID&decrypt=1',method:'GET',headers:{'Authorization':'Bearer $VTOKEN'}},res=>{let b='';res.on('data',d=>b+=d);res.on('end',()=>{console.log(JSON.parse(b).value)})});req.end();
")
echo "BLOB_READ_WRITE_TOKEN=$BLOB_TOKEN" >> .env.local

# Deploy preview
NODE_TLS_REJECT_UNAUTHORIZED=0 vercel deploy --yes
...
```bash
# Setup (simple summary)
git init && git add . && git commit -m "feat: initial setup"
"/c/Users/RachurA/AppData/Local/gh-cli/bin/gh.exe" repo create achyuthrachur/[name] --public --source=. --remote=origin --push
NODE_TLS_REJECT_UNAUTHORIZED=0 vercel link --yes --project [name]
NODE_TLS_REJECT_UNAUTHORIZED=0 vercel deploy --prod --yes
```
